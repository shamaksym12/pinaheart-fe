...
import {activeSubscription} from "../../actions/payment";
import {toggleModal} from "../../actions/ui";


const NineDayToAutoRenewal = ({ activeSubscription, toggleModal, active, user }) => {   
    return (
        <Fragment>
            <div>
                <div className='head'>
                ...
                <div className='para'>
                    <p>
                        ...
                        Your membership is about to expire  in 9 days. 
                        Make sure you don't  lose contact  with your  
                        future partnerand keep your conversations going
                         by  enabling  the Auto Renewal service.
                    </p>
                </div>

                <div className='buttons'>
                    <BtnMain style={{ marginBottom: '3rem' }}
                             className='btn-green' text='Enable Auto Renewal'
                             onClick={() => {
                                activeSubscription(active);
                                toggleModal(false, 'nineDayToAutoRenewal')
                             }}
                    />
                    <BtnMain style={{ marginBottom: '3rem' }}
                             className='btn-default' text='Cancel'
                             onClick={() => toggleModal(false, 'nineDayToAutoRenewal')}
                    />
                </div>
            </div>
            <style jsx>
...
const mapStateToProps = ({payment: { active }, user: { data }}) => {
    return {
      user: data,
      active,
    }
...
export default connect(mapStateToProps, mapDispatchToProps)(NineDayToAutoRenewal);