...
class ExpiredSubscription extends Component {
  ...
  render() {
    const { active, user, paymentList } = this.props
    const [ current ] = paymentList
    ...
    return (
      <Fragment>
          <div>
              ...
              <div className='para'>
                <p>
                  No panic. It takes just  one click to renew your  premium membership and enjoy again unlimited communications.
                </p>
                ...
                <p>
                  {
                    current ? 
                      `Your membership will be upgradeed to your previus status: ${ current.plan.name } membership  for ${ (current.plan.price / current.plan.unit_count).toFixed(2) } per month or a total of ${ current.plan.price } euro for the whole period.`
                      : ``
                  }                  
                </p>
              </div>
              ...
              <div className='buttons'>
                  <BtnMain style={{ marginBottom: '3rem' }}
                           className='btn-green' text='Renew My Membership'
                           onClick={() => {
                              toggleModal(false, 'expiredSubscription');
                              this.props.activeSubscription(active);
                           }}
                  />                    
              </div>

              <div>
                <a href="/settings/profile">Select another membership plan</a>
              </div>
          </div>
          <style jsx>
              {
...
const mapStateToProps = ({payment, user: { data }}) => {
    return {
      user: data,
